export const generateReportHTML = (data: any) => {
    ...
    // Valores dinámicos desde la configuración
    ...
    const prof_creds = config?.prof_creds || 'MSDS: 11330 / C.B.Z.: 2122';
    ...
    <div class="container">
        ...
        <div class="rounded-box patient-box">
            ...
            <div>FECHA: <span>${new Date().toLocaleDateString('es-VE')}</span></div>
        </div>
            ...
            <tbody>
                ${results.map((r: any) => `
                    <tr>
                        ...
                        <td>${r.unidad || '-'}</td>
                        <td style="font-size: 7.5pt;">${r.valor_min !== null && r.valor_min !== undefined ? `${r.valor_min} - ${r.valor_max}` : r.interpretacion || '-'}</td>
                    </tr>
                `).join('')}
            </tbody>
        </table>
